...
EventBrite <- function(write_file) {

  exec_dir <- paste0(getwd(), '/EB')
  ...
  #evURL<- paste0("https://www.eventbriteapi.com/v3/organizations/8536296706/events", token)
  all_event_info <- event_info(evURL)

  all_events <- lapply(all_event_info$uri, function(el) get_ppt_info(el, req_names, token)) # eSc_ppts_EB extracts info
  ...
  event_data <- merge(all_together, all_event_info, by="event_id")
  ...
  event_data <- left_join(event_data, unique(institutes), by="affiliation", all.x=T) %>%# do this again so the ones who filled out something like "PhD student"
    ...
    mutate(eSc_collab = eSc_collab=="Yes",
           eSc_collab = eSc_collab %in% TRUE) %>% #momice compatible
    mutate(order_id=as.character(order_id)) %>% #momice compatible
    mutate(car1 = str_replace_all(car1,
                                  c("Phd Candidate" = "Graduate Student",
                                    "Master Student" = "Graduate Student",
                                    "Researcher Software" = "Research Software",
                                    "Including" = "including"))) %>%
    arrange(.,year)
